...
describe("Phase 3 — Student Read-Only Profile & Verification Suite", () => {
  ...
  it("rejects non-image passport photo uploads", async () => {
    ...
    const invalidFile = new File(["hello"], "document.pdf", {
  ...
  it("2 & 3. Locked student sees Profile Locked banner and existing information", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(
          /Editing is disabled because your faculty mentor has locked this profile/i,
        ),
      ).toBeInTheDocument();
      expect(screen.getByDisplayValue("Anish Bezalwar")).toBeInTheDocument();
    });
  ...
  it("9. Locked student can navigate through wizard steps using Next/Previous", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Academic Information - Before Admission/i),
      ).toBeInTheDocument();
    });
  });
  ...
  it("11. Gracefully handles 403 PROFILE_LOCKED race condition from backend update", async () => {
    ...
    vi.mocked(updateProfile).mockRejectedValue(
      new Error("PROFILE_LOCKED: Your profile is locked."),
    );
    ...
    store.dispatch(
      studentProfileActions.patchStudentProfileData({
        is_profile_locked: true,
      }),
    );
