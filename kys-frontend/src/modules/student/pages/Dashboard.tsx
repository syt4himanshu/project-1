...
export default function Dashboard() {
  ...
  const draftKey = `kys_student_profile_draft_${
  ...
  const studentName = useMemo(() => {
    ...
    const pInfoName = (
      (profile as Record<string, unknown>)?.personal_info as Record<
        string,
        unknown
      >
    )?.full_name;
    if (typeof pInfoName === "string" && pInfoName.trim())
      return pInfoName.trim();
  ...
  return (
    <div className="student-dashboard min-h-screen bg-[var(--bg)] text-[var(--text)] transition-colors duration-300">
      <header
        ...
        style={{
          background: "linear-gradient(to right, #10284a, #17365f, #23497f)",
        }}
      >
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-3 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs uppercase tracking-[0.2em] text-[#b8c8df]">
                Know Your Student
              </p>
            </div>
          </div>
      ...
      <section className="border-b-2 border-[#f0c36b] bg-[linear-gradient(145deg,#263c67_0%,#334a7a_55%,#3e588e_100%)]">
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-8 sm:px-6 md:flex-row md:items-center md:justify-between lg:px-8">
          <div className="space-y-2">
            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[#b9c6dc]">
              Welcome Back
            </p>
            ...
            <p className="max-w-2xl text-sm text-[#d4ddee] sm:text-base">
              Manage your profile, track mentoring sessions and monitor
              progress.
            </p>
          </div>
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        {profile?.is_profile_locked && (
          <div className="mb-6 rounded-2xl border border-amber-300 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 p-4 text-amber-900 dark:text-amber-200">
            ...
            <p className="mt-1 text-sm text-amber-800 dark:text-amber-300">
              Your faculty mentor has locked your profile. You can view your
              profile, but you cannot edit it while it is locked.
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <section
            ...
            className="rounded-3xl border border-slate-200 dark:border-slate-800/60 bg-[var(--panel)] p-6 shadow-sm"
          >
            <p className="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-[#4f6ea1]">
              Profile
            </p>
            ...
            <div
              data-dashboard-border="soft"
              className="mt-6 rounded-2xl border border-slate-200 dark:border-slate-800/60 bg-[var(--bg)]/40 p-4"
            >
              <div className="flex items-center gap-3">
          ...
          <section
            ...
            className="rounded-3xl border border-slate-200 dark:border-slate-800/60 bg-[var(--panel)] p-6 shadow-sm"
          >
            <p className="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-[#a37200]">
              Mentor
            </p>
            <h2 className="font-serif text-3xl font-semibold text-[var(--text-h)]">
              Mentor Information
            </h2>
            <p className="mt-1 text-sm text-[var(--text-muted)]">
              Your assigned faculty mentor details
            </p>
              ...
              {loadingMentor ? (
                ...
                <>
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div
                      data-dashboard-border="soft"
                      className="rounded-xl border border-slate-200 dark:border-slate-800/60 bg-[var(--bg)]/45 p-3"
                    >
                      <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--text-muted)]">
                        Email
                      </p>
                      <p className="mt-1 break-all text-sm text-[var(--text-h)]">
                        {mentor.email}
                      </p>
                    </div>
                    <div
          ...
          <section
            data-dashboard-border="soft"
            className="rounded-3xl border border-slate-200 dark:border-slate-800/60 bg-[var(--panel)] p-6 shadow-sm lg:col-span-2"
          >
            <p className="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-[#3f78dc]">
              Mentoring
            </p>
            <h2 className="font-serif text-3xl font-semibold text-[var(--text-h)]">
              Mentoring Remarks
            ...
            <p className="mt-1 text-sm text-[var(--text-muted)]">
              Feedback and suggestions from your mentors
            </p>

            <div
              data-dashboard-border="soft"
              className="mt-6 rounded-2xl border border-slate-200 dark:border-slate-800/60 bg-[var(--bg)]/40 p-4 sm:p-5"
            >
              {loadingMinutes ? (
                ...
                <div className="py-12 text-center">
                  <p className="font-serif text-3xl text-[var(--text-h)]">
                    No Mentoring Sessions Yet
                  </p>
                  <p className="mx-auto mt-3 max-w-xl text-sm text-[var(--text-muted)]">
                    Your remarks will appear here once you have sessions with
                    your faculty mentor.
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  {minutes.map((m) => {
                    const highlights = extractHighlights(
                      [m.remarks, m.mentor_remarks].filter(Boolean).join(" "),
                    );
                    return (
                      <article
                        data-dashboard-border="soft"
                        key={m.id}
                        className="rounded-2xl border border-slate-200 dark:border-slate-800/60 bg-[var(--panel)] p-4"
                      >
                        ...
                        <div className="space-y-2 text-sm text-[var(--text)]">
                          <p>
                            <span className="font-semibold">AI Remarks:</span>{" "}
                            {m.remarks || "None"}
                          </p>
                          <p>
                            <span className="font-semibold">
      ...
      {showLogoutModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 px-4 py-6">
          <div className="w-full max-w-xl rounded-[22px] border border-[#d5dcea] bg-[#f7f9fc] p-6 shadow-[0_28px_60px_-25px_rgba(17,28,48,0.55)] sm:p-7 dark:border-[#334155] dark:bg-[#1e293b]">
            ...
            <h3 className="font-serif text-4xl font-semibold text-[#1f304d] dark:text-gray-100">
              Log Out
            </h3>
            <p className="mt-2 text-lg text-[#6f7f96] dark:text-gray-300">
