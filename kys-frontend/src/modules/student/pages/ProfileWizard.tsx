...
export default function ProfileWizard() {
  ...
  useEffect(() => {
    if (
      ...
      window.history.state.wizardStep === undefined
    ) {
  ...
  const handleSubmit = async () => {
    ...
    if (submitted) {
      toast.success(
        "Your mentoring profile has been successfully submitted.",
        "Profile Submitted",
      );
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-soft)] px-3 py-5 sm:px-4 sm:py-8 transition-colors duration-300">
      <div
        className="mx-auto w-full max-w-5xl overflow-hidden rounded-3xl bg-[var(--bg-soft)] shadow-[0_20px_45px_-24px_rgba(22,42,72,0.45)]"
        style={{ border: "1px solid var(--outer-border)" }}
      >
        <header className="border-t-[3px] border-[#f0b243] bg-white px-5 py-4 sm:px-8 sm:py-5 flex justify-between items-start gap-4 dark:bg-slate-800">
          <div className="flex-1 flex flex-col gap-3 sm:gap-4 min-w-0">
            <img
              src="/logo.png"
              alt="Logo"
              className="h-16 sm:h-30 object-contain self-center object-center"
            />
            <div>
              <h1
            ...
            {STEPS.slice(0, PROGRESS_STEPS).map((_, i) => {
              ...
              return (
                ...
                >
                  ...
                  {i < PROGRESS_STEPS - 1 && (
                    <div className="mx-2 h-px flex-1 bg-[#d8dfeb] sm:mx-3" />
                  )}
                </div>
              );
            })}
            ...
            <div className="mt-3 rounded-2xl border border-[#dbe5f2] bg-[#f7fbff] px-4 py-3 text-sm text-[#324a6b]">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="draft-banner__meta mt-1 text-xs">
                    Last saved: {new Date(draftRestoredAt).toLocaleString()}
                  </p>
                </div>
                <button
                  type="button"
        ...
        <main className="px-5 py-6 sm:px-8 sm:py-8">
          {isLocked && (
            <div className="mb-6 rounded-2xl border border-amber-300 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 p-4 text-amber-900 dark:text-amber-200">
              ...
              <p className="mt-1 text-sm text-amber-800 dark:text-amber-300">
                Editing is disabled because your faculty mentor has locked this
                profile. You can view your information, but modifications are
                restricted.
              </p>
            </div>
          )}
          ...
          <div className="mb-6">
            <h2 className="font-serif text-3xl font-semibold text-[var(--text)] sm:text-4xl">
              {STEPS[step]}
            </h2>
              ...
              {!isLocked && (
                <button
                  ...
                  onClick={async () => {
                    if (
                      window.confirm(
                        "Clear all filled form data and start over?",
                      )
                    ) {
                      await clearForm();
                    }
