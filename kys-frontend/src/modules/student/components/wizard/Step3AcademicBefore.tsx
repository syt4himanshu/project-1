...
const YEAR_OPTIONS = Array.from({ length: 2040 - 2018 + 1 }, (_, i) =>
  String(2018 + i),
);
...
export default function Step3AcademicBefore() {
  ...
  const records =
    (data.past_education_records as Record<string, unknown>[]) || [];
  ...
  const getRecord = (exam: string) =>
    records.find((r) => r.exam_name === exam) || {};
  ...
  const upd = (exam: string, key: string, value: unknown) => {
    ...
    update({
      past_education_records: [
        ...existing,
        { ...current, exam_name: exam, [key]: value },
      ],
    });
  };
  ...
  const renderEducationSection = (
    ...
    const isCustomBoard =
      rec.board &&
      !BOARDS.includes(rec.board as string) &&
      rec.board !== "Other";
      ...
      <section key={examKey} className={sectionCardCls}>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 sm:gap-5">
          {field(
            ...
            useBoardInput ? (
              input(
                "text",
                (rec.board as string) || "",
                (v) => upd(examKey, "board", v),
                boardPlaceholder,
                getValidation(`${examKey} Board`),
              )
            ) : (
              <div className="flex flex-col gap-4">
                ...
                {showBoardInput &&
                  input(
                    "text",
                    isCustomBoard ? (rec.board as string) : "",
                    (v) => upd(examKey, "board", v),
                    `Enter ${boardLabel}`,
                    getValidation(`${examKey} Board`),
                  )}
          ...
          {field(
            ...
            input(
              "text",
              rec.percentage != null ? String(rec.percentage) : "",
              (v) => handlePercentageChange(examKey, v),
              "e.g. 85.50",
              getValidation(`${examKey} Percentage / Grade`),
            ),
          )}
          {field(
            "Year of Passing *",
            select(
              YEAR_OPTIONS,
              String(rec.year_of_passing || ""),
              (v) =>
                upd(examKey, "year_of_passing", v === "" ? null : Number(v)),
              "Select Year",
              getValidation(`${examKey} Year of Passing`),
            ),
          )}
        </div>
      </section>
  ...
  const updPostAdmission = (sem: number, key: string, value: unknown) => {
    const existing = postAdmissionRecords.filter(
      (r) => Number(r.semester) !== sem,
    );
    const current = getPostAdmissionRecord(sem);
    update({
      post_admission_records: [
        ...existing,
        { ...current, semester: sem, [key]: value },
      ],
    });
  };
            ...
            {semesters.slice(0, activeSem).map((sem) => {
              ...
              return (
                <section
                  key={sem}
                  ref={(el) => {
                    semCardRefs.current[sem] = el;
                  }}
                  className={sectionCardCls}
                >
                  <h3 className="mb-4 text-xl font-semibold text-[#223b60]">
                    Semester {sem}
                  </h3>
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5">
                    ...
                    {field(
                      ...
                      input(
                        "text",
                        (rec.college_rank as string) || "",
                        (v) => updPostAdmission(sem, "college_rank", v),
                        "Rank (if any)",
                      ),
                    )}
                    {field(
                      ...
                      input(
                        "text",
                        (rec.academic_awards as string) || "",
                        (v) => updPostAdmission(sem, "academic_awards", v),
                    ...
                    <div className="sm:col-span-2">
                      {field(
                        "Backlog Subjects and Practical labs",
                        input(
                          "text",
                          (rec.backlog_subjects as string) || "",
                          (v) => updPostAdmission(sem, "backlog_subjects", v),
                          "e.g. list subjects comma separated",
                        ),
                      )}
                    </div>
                  </div>
                    ...
                    <div className="mt-3">
                      {activeSem < semesters.length ? (
                        ...
                        <p className="text-sm font-medium text-[#12996c]">
                          ✓ All {semesters.length} semesters added
                        </p>
                      )}
                    </div>
                  )}
