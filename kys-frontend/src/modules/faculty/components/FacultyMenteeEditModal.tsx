...
const SECTIONS: Array<{
  ...
  anchor: string
  icon: typeof User
  countKey?: 'projects' | 'internships' | 'participation' | 'organization' | 'skill-programs' | 'academics'
}> = [
    { id: 'personal', label: 'Personal Details', description: 'Basic info, contact, address and photo', anchor: 'profile-section-personal', icon: User },
    { id: 'parents', label: 'Parents & Guardian', description: 'Father, mother and local guardian details', anchor: 'profile-section-parents', icon: Users },
    { id: 'emergency', label: 'Emergency Contact', description: 'Emergency contact person and number', anchor: 'profile-section-emergency', icon: Phone },
    { id: 'past-education', label: 'Past Education', description: 'SSC, HSC / Diploma, entrance exam records', anchor: 'profile-section-past-education-root', icon: BookOpen },
    { id: 'academics', label: 'Academic Records', description: 'Semester-wise SGPA, backlogs and awards', anchor: 'profile-section-academics', icon: GraduationCap, countKey: 'academics' },
    { id: 'projects', label: 'Projects', description: 'Mini, major and collaborative projects', anchor: 'profile-section-projects', icon: FolderKanban, countKey: 'projects' },
    { id: 'internships', label: 'Internships', description: 'Internship experience with company and role details', anchor: 'profile-section-internships', icon: Briefcase, countKey: 'internships' },
    { id: 'participation', label: 'Co-Curricular Activities', description: 'Competition and hackathon participation', anchor: 'profile-section-participation', icon: Award, countKey: 'participation' },
    { id: 'organization', label: 'Co-Curricular Organization', description: 'Events organised or co-ordinated', anchor: 'profile-section-organization', icon: Users, countKey: 'organization' },
    { id: 'skill-programs', label: 'Skill Programs', description: 'SDP, MOOC, training and certification courses', anchor: 'profile-section-skill-programs', icon: Sparkles, countKey: 'skill-programs' },
    { id: 'career', label: 'Career Objective', description: 'Career goals, placement interest and expectations', anchor: 'profile-section-career', icon: Target },
    { id: 'skills', label: 'Skills', description: 'Technical and soft skills, tools and frameworks', anchor: 'profile-section-skills', icon: Sparkles },
    { id: 'swoc', label: 'SWOC', description: 'Strengths, weaknesses, opportunities and challenges', anchor: 'profile-section-swoc', icon: Target },
  ]
...
const SECTION_ANCHOR_MAP: Record<SectionId, string[]> = {
  'personal': ['profile-section-personal', 'profile-section-location', 'profile-section-photo'],
  ...
  'internships': ['profile-section-internships', 'profile-section-cocurricular-participation'],
...
function normalizePersonalInfoForDraft(raw: unknown): Record<string, unknown> {
  ...
  const photoUrl = extractStudentPhotoUrl({ personal_info: pi }) ?? pi.photoUrl ?? pi.photo_url ?? null
  const preview =
    extractStudentPhotoPreviewUrl({ personal_info: pi }) ??
    pi.photoPreviewUrl ??
    pi.photo_preview_url ??
    null
  if (photoUrl) { pi.photoUrl = photoUrl; pi.photo_url = photoUrl }
  if (preview) { pi.photoPreviewUrl = preview; pi.photo_preview_url = preview }
...
function buildDraftFromMentee(mentee: MenteePayload): Record<string, unknown> {
  const projects = Array.isArray(mentee.projects) ? (cloneData(mentee.projects) as unknown[]) : []
  while (projects.length < 3) projects.push({})

  const internships = Array.isArray(mentee.internships) ? (cloneData(mentee.internships) as unknown[]) : []
  const organizations = Array.isArray(mentee.cocurricular_organizations) ? (cloneData(mentee.cocurricular_organizations) as unknown[]) : []
  const skillPrograms = Array.isArray(mentee.skill_programs) ? (cloneData(mentee.skill_programs) as unknown[]) : []
  ...
  const hasInternshipExperience = internships.some((item) => {
    ...
    return Boolean(row.company_name || row.designation || row.domain || row.description || row.internship_type || row.paid_unpaid || row.start_date || row.end_date || row.title)
  })
  ...
  const hasSkillPrograms = skillPrograms.some((item) => {
    const row = (item ?? {}) as Record<string, unknown>
    return Boolean(row.course_title || row.platform || row.duration_hours || row.date_from || row.date_to)
  })
  ...
  return {
    ...
    cocurricular_participations: cloneData(mentee.cocurricular_participations || [{}]),
...
function buildSavePayload(data: Record<string, unknown>): Record<string, unknown> {
...
function patchDraft(prev: Record<string, unknown>, patch: Record<string, unknown>): Record<string, unknown> {
  const next: Record<string, unknown> = { ...prev }
  for (const [key, value] of Object.entries(patch)) {
    if (isRecord(value)) {
      next[key] = { ...((isRecord(prev[key]) ? prev[key] : {}) as Record<string, unknown>), ...value }
    } else {
      next[key] = value
    }
  }
...
function PreviewPanel({ data }: { data: ProfileDraftMap }) {
  ...
  const row = (label: string, value: unknown) => (
    <div key={label} className="faculty-profile-editor__preview-row">
      <dt>{label}</dt>
      <dd>{value == null || String(value).trim() === '' ? 'N/A' : String(value)}</dd>
    </div>
  )
  ...
  return (
    <div className="faculty-profile-editor__preview">
      <p className="faculty-profile-editor__preview-note">
        Read-only preview of the current draft. Nothing is saved until you click Save Profile Changes.
      </p>
      <section>
        <h4>Personal</h4>
        <dl>
      ...
      <section>
        ...
        <ul>
          {projects.map((item, index) => {
            ...
            return <li key={`preview-project-${index}`}>{String(p.title || `Project ${index + 1}`)} — {String(p.domain || 'N/A')}</li>
          })}
        </ul>
      </section>
      <section>
        ...
        <ul>
          {internships.map((item, index) => {
            ...
            return <li key={`preview-internship-${index}`}>{String(intern.company_name || intern.title || `Internship ${index + 1}`)}</li>
          })}
        </ul>
      </section>
      <section>
...
function FacultyMenteeEditForm({
  ...
  const [baselineSignature, setBaselineSignature] = useState(() => JSON.stringify(buildDraftFromMentee(mentee)))
  ...
  const uploadPhoto = useCallback(
    async (file: File): Promise<ProfilePhotoUploadResult> => {
      const response = (await uploadMutation.mutateAsync(file)) as unknown as Record<string, unknown>
      return {
        photoUrl: (response.photoUrl as string) ?? (response.photo_url as string) ?? null,
        photo_public_id: (response.photo_public_id as string) ?? null,
        photo_preview_url: (response.photo_preview_url as string) ?? (response.photoPreviewUrl as string) ?? null,
        photoPreviewUrl: (response.photoPreviewUrl as string) ?? (response.photo_preview_url as string) ?? null,
      }
    },
    [uploadMutation],
  )
  ...
  const draftContextValue = useMemo(
    ...
    [draft, formError, getFieldValidation, markFieldTouched, update, uploadPhoto, validationErrors],
  )

  const navigateSection = (id: SectionId) => {
    setActiveSection(id)
    // Scroll the content pane to top when switching sections
    const contentEl = document.querySelector('.faculty-profile-editor__content')
    if (contentEl) contentEl.scrollTop = 0
  }

  const goToPrevSection = () => {
    if (activeSectionIndex > 0) navigateSection(SECTIONS[activeSectionIndex - 1].id)
  }

  const goToNextSection = () => {
    if (activeSectionIndex < SECTIONS.length - 1) navigateSection(SECTIONS[activeSectionIndex + 1].id)
  }
  ...
  useEffect(() => {
    const onKeydown = (event: KeyboardEvent) => {
      ...
      if (discardOpen) { setDiscardOpen(false); return }
      if (updateMutation.isPending) return
      if (isDirty) { setDiscardOpen(true); return }
      onClose()
    }
  ...
  const handleSubmit = async (event?: FormEvent) => {
    ...
    if (!validation.isValid) {
      ...
      setFormError(validation.errors[0] || 'Please fix validation errors before saving.')
      return
    }
    try {
      ...
      setFormError(toApiErrorMessage(error, 'Failed to update mentee profile.'))
    }
  }
  ...
  const photoPreview =
    extractStudentPhotoPreviewUrl({ personal_info: pi }) ||
    (typeof pi.photoPreviewUrl === 'string' ? pi.photoPreviewUrl : null) ||
    (typeof pi.photo_preview_url === 'string' ? pi.photo_preview_url : null)
  ...
  return (
    <ProfileDraftProvider value={draftContextValue}>
      ...
      <div className="faculty-profile-editor" role="dialog" aria-modal="true" aria-label="Edit Mentee Profile">
        <div className="faculty-profile-editor__backdrop" onClick={requestClose} />
        ...
        <div className="faculty-profile-editor__panel">
          ...
          <header className="faculty-profile-editor__header">
            <div className="faculty-profile-editor__identity">
              <div className="faculty-profile-editor__avatar" aria-hidden="true">
                {photoPreview ? (
                  <img src={photoPreview} alt="" />
                ) : (
                  <span>{String(draft.full_name || 'ST').slice(0, 2).toUpperCase()}</span>
                )}
              </div>
              <div className="faculty-profile-editor__identity-text">
                <div className="faculty-profile-editor__title-row">
                  ...
                  <button type="button" className="faculty-profile-editor__icon-btn" onClick={requestClose} aria-label="Close editor">
                    <X size={18} />
                  </button>
                </div>
                <p className="faculty-profile-editor__name">{String(draft.full_name || 'Unnamed Student')}</p>
                ...
                <div className={`faculty-profile-editor__lock ${draft.is_profile_locked ? 'faculty-profile-editor__lock--locked' : 'faculty-profile-editor__lock--open'}`}>
                  <Lock size={14} aria-hidden="true" />
                  <div>
                    <strong>{draft.is_profile_locked ? 'Profile Locked' : 'Editable by Student'}</strong>
                    <p>
          ...
          <div className="faculty-profile-editor__body">
            {/* Sidebar navigation */}
            <nav className="faculty-profile-editor__sidebar" aria-label="Profile sections">
              {SECTIONS.map((section) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="faculty-profile-editor__nav-label">{section.label}</span>
                    {count != null && count > 0 ? (
                      <span className="faculty-profile-editor__nav-count">{count}</span>
                    ) : null}
                  </button>
                )
              })}
            ...
            {/* Main content area */}
            <div className="faculty-profile-editor__content" aria-live="polite" aria-atomic="false">
              {/* Section heading */}
              ...
              <form id="faculty-mentee-profile-form" onSubmit={(e) => void handleSubmit(e)}>
          ...
          {(formError || validationErrors.length > 0) && (
            <div className="faculty-profile-editor__errors" role="alert">
              ...
              <div>
                <p>{formError || 'Please fix the highlighted validation issues.'}</p>
          ...
          <footer className="faculty-profile-editor__footer">
            <button type="button" className="button button--soft" onClick={requestClose} disabled={updateMutation.isPending}>
              Cancel
            </button>
            <button type="button" className="button button--soft" onClick={() => setPreviewOpen(true)} disabled={updateMutation.isPending}>
              <Eye size={16} style={{ marginRight: 6 }} aria-hidden="true" />
              Preview
            </button>
            <button type="submit" form="faculty-mentee-profile-form" className="button button--primary" disabled={updateMutation.isPending}>
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-3">
            <button type="button" className="button button--soft" onClick={() => setDiscardOpen(false)}>Stay</button>
            <button type="button" className="button button--danger" onClick={() => { setDiscardOpen(false); onClose() }}>
              Discard Changes
            </button>
          </div>
        }
      >
        <p style={{ margin: 0 }}>Closing now will discard edits you made to this mentee profile.</p>
      </Modal>
      ...
      <Modal
        ...
        size="xl"
        footer={
          <button type="button" className="button button--soft" onClick={() => setPreviewOpen(false)}>
            Close Preview
          </button>
        }
      >
...
export function FacultyMenteeEditModal({ uid, open, mentee, onClose }: FacultyMenteeEditModalProps) {
