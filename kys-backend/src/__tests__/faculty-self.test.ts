...
describe('faculty self APIs', () => {
  ...
  it('GET /faculty/me', async () => {
    const ok = await request<any>('GET', '/api/faculty/me', undefined, facultyToken);
    expect(ok.status).toBe(200);
    expect(ok.body).toMatchObject({ email: expect.any(String) });

    const student = await request('GET', '/api/faculty/me', undefined, studentToken);
    expect(student.status).toBe(403);

    const noToken = await request('GET', '/api/faculty/me');
    expect(noToken.status).toBe(401);
  });

  it('PUT /faculty/me update first_name/contact and invalid domain check', async () => {
    const up1 = await request('PUT', '/api/faculty/me', { first_name: 'UpdatedFaculty' }, facultyToken);
    expect(up1.status).toBe(200);

    const up2 = await request('PUT', '/api/faculty/me', { contact_number: '8888888888' }, facultyToken);
    expect(up2.status).toBe(200);

    const bad = await request('PUT', '/api/faculty/me', { email: 'bad@gmail.com' }, facultyToken);
    expect([200, 400]).toContain(bad.status);

    const byStudent = await request('PUT', '/api/faculty/me', { first_name: 'x' }, studentToken);
    expect(byStudent.status).toBe(403);
  });

  it('GET /faculty/me/mentees returns assigned only', async () => {
    const res = await request<any[]>('GET', '/api/faculty/me/mentees', undefined, facultyToken);
    ...
    const byStudent = await request('GET', '/api/faculty/me/mentees', undefined, studentToken);
    expect(byStudent.status).toBe(403);
  });

  it('POST /api/faculty/me/mentees/:uid/minutes', async () => {
    const targetUid = assignedUids[0];
    const good = await request(
      'POST',
      `/api/faculty/me/mentees/${targetUid}/minutes`,
      {
        semester: 3,
        date: '2025-01-15',
    ...
    const list = await request<any>('GET', `/api/faculty/me/mentees/${targetUid}/minutes`, undefined, facultyToken);
    expect([200, 404]).toContain(list.status);

    const notAssigned = await request(
      'POST',
      `/api/faculty/me/mentees/${unassignedUid}/minutes`,
      { remarks: 'Should fail' },
      facultyToken,
    );
    ...
    const nonExistent = await request(
      'POST',
      '/api/faculty/me/mentees/NO_SUCH_UID/minutes',
      { remarks: 'Should fail' },
      facultyToken,
    );
    expect(nonExistent.status).toBe(404);

    const missingRemarks = await request('POST', `/api/faculty/me/mentees/${targetUid}/minutes`, { suggestion: 'x' }, facultyToken);
    expect(missingRemarks.status).toBe(400);

    const byStudent = await request('POST', `/api/faculty/me/mentees/${targetUid}/minutes`, { remarks: 'x' }, studentToken);
    expect(byStudent.status).toBe(403);
  });

  it('GET /api/faculty/me/mentees/:uid/minutes ordering and access', async () => {
    const targetUid = assignedUids[1];

    for (let i = 0; i < 3; i += 1) {
      await request('POST', `/api/faculty/me/mentees/${targetUid}/minutes`, { remarks: `Minute ${i}`, suggestion: 's', action: 'a' }, facultyToken);
    }

    const res = await request<any>('GET', `/api/faculty/me/mentees/${targetUid}/minutes`, undefined, facultyToken);
    ...
    const notAssigned = await request('GET', `/api/faculty/me/mentees/${unassignedUid}/minutes`, undefined, facultyToken);
    expect([403, 404]).toContain(notAssigned.status);

    const byStudent = await request('GET', `/api/faculty/me/mentees/${targetUid}/minutes`, undefined, studentToken);
    expect(byStudent.status).toBe(403);
  });
});
