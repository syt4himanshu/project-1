...
function normalizeActiveBacklogCount(records: AnyRecord[], source?: AnyRecord) {
    const directCandidates = [source?.number_of_active_backlogs, source?.active_backlogs, source?.current_backlogs]
    ...
    const latest = [...records].sort((a, b) => Number(a.semester ?? 0) - Number(b.semester ?? 0)).at(-1)
...
export default function StudentDetailDialog({ studentId, onClose }: Props) {
    ...
    const studentRecord = useMemo(() => (student as AnyRecord | undefined) ?? undefined, [student])
    ...
    const handlePrint = async () => {
        ...
        try {
            ...
            printWindow.document.write(`
                <html>
                    <head>
                        <title>${showValue(studentRecord?.name ?? studentRecord?.uid ?? 'Student')} - Profile</title>
                        ${styleTags}
                        <style>
    ...
    const handlePDF = async () => {
        ...
        try {
            ...
            const boundaries = uniqueSorted(
                boundaryElements
                    .map((el) => {
                        ...
                        return Math.max(0, Math.min(canvas.height, Math.round(relativeBottom * scaleY)))
                    })
            ...
            while (cursor < canvas.height - 1) {
                ...
                const candidateCuts = boundaries.filter((y) => y > cursor && y <= targetEnd)
                const cutEnd = candidateCuts.length ? candidateCuts[candidateCuts.length - 1] : targetEnd
                ...
                pdf.addImage(pageImg, 'PNG', marginX, marginTop, imageWidth, sliceHeightMm)

                const nextCursor = cutPoint > cursor ? cutPoint : Math.min(canvas.height, cursor + pagePixelHeight)
                if (nextCursor <= cursor) break
                cursor = nextCursor
                pageIndex += 1
            }

            pdf.save(`${showValue(studentRecord?.name ?? studentRecord?.uid ?? 'student')}-full-profile.pdf`)
        } finally {
            if (printRef.current) printRef.current.classList.remove('pdf-export-mode')
            cleanupFullContent()
            setExporting(false)
        }
    }
    ...
    const backlogSubjects = latestAcademicRecord
        ...
        : Array.from(new Set(academicRecords.flatMap((record) => extractBacklogSubjects(record))))
        ...
        <Dialog open={!!studentId} onOpenChange={(v) => { if (!v) onClose() }}>
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto p-0 [&>button]:hidden">
                        ...
                        {isLoading ? (
                            ...
                            <div className="flex justify-between items-center w-full gap-4">
                                <div className="flex-shrink-0 flex items-center justify-start max-w-[120px] ml-6">
                                    <img src="/college-logo.png" alt="College Logo" className="w-24 h-24 object-contain" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none' }} />
                                </div>
                                <div className="flex-col flex items-center justify-center text-center px-4 flex-1">
                                    <h2 className="text-[1.35rem] sm:text-2xl font-extrabold text-slate-900 tracking-tight uppercase mb-1">Student Information</h2>
                                    <p className="text-slate-700 font-semibold text-sm sm:text-base mb-0.5">Department of Computer Science Engineering</p>
                                    <p className="text-slate-500 font-medium text-xs sm:text-sm">{SESSION_LABEL}</p>
                                </div>
                                <div className="flex-shrink-0 flex items-center justify-end min-w-[96px] mr-6">
                                    <div className="w-24 h-[120px] border-[3px] border-slate-100 rounded-xl bg-slate-50 flex items-center justify-center overflow-hidden shadow-sm">
                                        {!isEmpty(pick(pi, 'profile_photo', 'photo_url')) ? (
                                            <img src={showValue(pick(pi, 'profile_photo', 'photo_url'))} alt="Profile" className="w-full h-full object-cover" />
                                        ) : (
                            ...
                            <>
                                <CollapsibleSection title="Student's Personal Information" defaultOpen forceOpen={forceOpenForExport}>
                                    <InfoTable rows={[
                                        ...
                                        { label: 'Roll No./UID', value: showValue(pick(pi, 'roll_no', 'roll_number', 'mis_uid') || student.uid) },
                                        { label: 'Year of Admission', value: showValue(student.year_of_admission) },
                                        { label: 'Date of Birth', value: isEmpty(pi?.dob) ? 'N/A' : formatDate(textValue(pi?.dob)) },
                                        { label: 'Gender', value: showValue(pi?.gender) },
                                        { label: 'Blood Group', value: showValue(pi?.blood_group) },
                                        { label: 'Category', value: showValue(pi?.category) },
                                        { label: 'Aadhar Card Number', value: showValue(pick(pi, 'aadhar', 'aadhar_number')) },
                                        { label: 'MIS UID', value: showValue(pick(pi, 'mis_uid', 'roll_no', 'roll_number', 'uid')) },
                                        { label: 'Mobile No.', value: showValue(pick(pi, 'mobile', 'mobile_no')) },
                                        { label: 'Personal Email ID', value: showValue(pi?.personal_email) },
                                        { label: 'College Email ID', value: showValue(pi?.college_email) },
                                        { label: 'LinkedIn ID', value: showValue(pick(pi, 'linkedin', 'linked_in_id')) },
                                        { label: 'GitHub ID', value: showValue(pick(pi, 'github', 'github_id')) },
                                        { label: 'Permanent Address', value: showValue(pick(pi, 'permanent_address', 'address')) },
                                        { label: 'Present Address', value: showValue(pi?.present_address) },
                                        { label: 'Local Guardian Name', value: showValue(pi?.local_guardian_name) },
                                        { label: 'Local Guardian Mobile', value: showValue(pi?.local_guardian_mobile) },
                                        { label: 'Local Guardian Email', value: showValue(pi?.local_guardian_email) },
                                    ]} />
                                </CollapsibleSection>

                                <CollapsibleSection title="Parent's Information" defaultOpen forceOpen={forceOpenForExport}>
                                    <InfoTable rows={[
                                        { label: "Father's Name", value: showValue(pi?.father_name) },
                                        { label: "Father's Mobile No.", value: showValue(pick(pi, 'father_mobile', 'father_mobile_no')) },
                                        { label: "Father's Email ID", value: showValue(pi?.father_email) },
                                        { label: "Father's Occupation", value: showValue(pi?.father_occupation) },
                                        { label: "Mother's Name", value: showValue(pi?.mother_name) },
                                        { label: "Mother's Mobile No.", value: showValue(pick(pi, 'mother_mobile', 'mother_mobile_no')) },
                                        { label: "Mother's Email ID", value: showValue(pi?.mother_email) },
                                        { label: "Mother's Occupation", value: showValue(pi?.mother_occupation) },
                                        { label: 'Emergency Contact', value: showValue(pick(pi, 'emergency_contact', 'emergency_contact_number')) },
                                    ]} />
                                </CollapsibleSection>
                                ...
                                <CollapsibleSection title="Post-Admission Academic Record" forceOpen={forceOpenForExport}>
                                ...
                                <CollapsibleSection title="SWOC Analysis" forceOpen={forceOpenForExport}>
                                    <div className="grid grid-cols-2 gap-3">
                                        {[
                                            ...
                                            <div key={key} className={`bg-${color}-50 border border-${color}-200 rounded-lg p-3`}>
                                                ...
                                                <p className="text-sm text-slate-700">{showValue(swoc?.[key])}</p>
                                            </div>
                                        ))}
                                    </div>
                                </CollapsibleSection>
                                ...
                                <CollapsibleSection title="Assigned Mentor" forceOpen={forceOpenForExport}>
                                    {student.mentor_name ? (
                ...
                <div className="sticky bottom-0 border-t border-slate-200 bg-white px-5 py-3 flex gap-2 justify-end">
                    <Button variant="outline" size="sm" onClick={handlePrint} disabled={isLoading || exporting || !student}>
                        <Printer className="w-4 h-4 mr-1" /> Print
                    </Button>
                    <Button size="sm" onClick={handlePDF} disabled={isLoading || exporting || !student}>
